...
export function loadPosts(currentPage) {

    const apiService = new ApiService();
    let answer
    let param = location.pathname.split("/")[-1]

    answer = apiService.GetPostsOnPage(currentPage);
    answer.then((data) => {
        if (data.body) {
            $("#posts").empty();
            addPostsCards($("#posts"), data.body.posts, $("#card-template"));
            addPaginationItems(currentPage, data.body.pagination.count, "");
        }
    });
}
...
function addPaginationItems(currentPage, pageCount, url) {
    ...
    pagination.append(`
        <li class="page-item direction-item">
            <a class="page-link" href="/${url}${currentPage - 1 >= 1 ? currentPage - 1 : 1}" tabindex="-1" aria-disabled="true" data-link><i class="fa-solid fa-angle-left"></i></a>
        </li>
    `);
    ...
    pagination.append(`
        <li class="page-item direction-item">
            <a class="page-link" href="/${url}${currentPage + 1 <= pageCount ? currentPage + 1 : pageCount}" data-link><i class="fa-solid fa-angle-right"></i></a>
        </li>
    `);
}
